...
class ClsModel(YTMTNetBase):
    ...
    def initialize(self, opt):
        ...
        self.net_c.load_state_dict(torch.load('/root/lanyun-tmp/RDNet/pretrained/cls_model.pth')['icnn'])
        ...
        self.net_i.load_state_dict(torch.load('/root/lanyun-tmp/RDNet/pretrained/rdnetcon2_050_00019000.pt')['icnn'])
        self.net_w = RetinexFormer().to(self.device)                         
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(self.net_w.parameters(),   #修改为w的
    ...
    def forward(self):
        ...
        #print(self.output_i)
        for i in range(self.opt.loss_col):
            out_reflection, out_clean = output_j[i][:, :3, ...], output_j[i][:, 3:, ...]  #out_clean是干净的图像
            x = out_clean
            x1 = self.net_w(x)  # 输出形状需与 out_clean 一致
            refined_clean = (x + x1 * 0.05 ) / (1 + 0.05)
            self.output_j.append(refined_clean) 
    ...
    def forward_eval(self):
        ...
        #self.output_i = self.net_w(output_i)+output_i   #添加的第二阶段精细化去反射阶段
        for i in range(self.opt.loss_col):
    ...
    def load(self, model, resume_epoch=None):
        ...
        model.net_w.load_state_dict(state_dict['icnn'])
        return state_dict

    def state_dict(self):
        state_dict = {
            'icnn': self.net_w.state_dict(),
