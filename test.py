...
parser = argparse.ArgumentParser(description='SIRR')
parser.add_argument('--test_dir', type=str, default='./Alldata/test/blended')  
parser.add_argument('--output_dir', type=str, default='./Alldata/test/trans/')
...
try:
   MyEnsembleNet.load_state_dict(torch.load('./pretrained/dahazebest.pth'))
...
with torch.no_grad():
    for batch_idx, (hazy,name) in enumerate(test_loader):
        ...
        imwrite(frame_out, output_dir + '/' + 'test_'+str(name[0]) + '.jpg', range=(0, 1))
